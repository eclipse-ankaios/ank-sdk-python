...
def run_pytest_utest(args):
    ...
    result = pytest.main(
        [
            "--junitxml={}".format(
                os.path.join(UTEST_DIR, "utest_report.xml")
            ),
            "tests",
            # '-p', 'no:warnings',
            "-vv",
...
def run_pytest_cov(args):
    ...
    result = pytest.main(
        [
            ...
            "--cov-report=xml:{}".format(
                os.path.join(COVERAGE_DIR, "cov_report.xml")
            ),
            "--cov-report=term",
            "--cov-fail-under=100",
            "tests",
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-c", "--cov", action="store_true", help="Run coverage"
    )
    parser.add_argument(
        "-u", "--utest", action="store_true", help="Run unit tests"
    )
    parser.add_argument("-l", "--lint", action="store_true", help="Run pylint")
    parser.add_argument(
        "-p", "--pep8", action="store_true", help="Run pep8 codestyle check"
    )
