...
CONFIGS_PROTO = _ank_base.ConfigMap(
    configs={
        "config_1": _ank_base.ConfigItem(String="val_1"),
...
FILES_PROTO = _ank_base.Files(
    files=[
        _ank_base.File(mountPoint="./mount_point_1", data="data_1"),
        _ank_base.File(
            mountPoint="./mount_point_2", binaryData="binary_data_2"
        ),
    ]
)
...
def test_workload_functionality():
    ...
    assert (
        complete_state._complete_state.desiredState.workloads.workloads[
            "nginx_test"
        ]
        == wl_nginx._to_proto()
    )
...
def test_workload_states():
    ...
    """
    complete_state = CompleteState(
        _proto=_ank_base.CompleteState(workloadStates=WORKLOAD_STATES_PROTO)
    )
...
def test_get_agents():
    ...
    """
    complete_state = CompleteState(
        _proto=_ank_base.CompleteState(agents=AGENTS_PROTO)
    )
...
def test_get_configs():
    ...
    complete_state = CompleteState(
        _proto=_ank_base.CompleteState(
            desiredState=_ank_base.State(configs=CONFIGS_PROTO)
        )
    )
...
def test_from_manifest():
    ...
    assert complete_state.get_configs() == {"test_ports": {"port": "8081"}}
                ...
                "dynamic_nginx": {
                    ...
                    "files": [
                        ...
                        },
                        {
                            ...
                            "content": {"binaryData": "binary_data_1"},
                        },
                    ],
                    "configs": {
                        "array": "config_2",
