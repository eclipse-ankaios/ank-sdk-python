...
class Ankaios:
    ...
    def start_read(self):
        """Starts a thread to read from the control interface input fifo."""
        self.read_thread = threading.Thread(target=self._read_from_control_interface)
    ...
    def create_logger(self):
        ...
        self.change_logger_level(AnkaiosLogLevel.INFO)

    def set_logger_level(self, level: AnkaiosLogLevel):
        self.logger.setLevel(level)

    def create_request_to_add_new_workload(self):
    ...
    def create_request_for_complete_state(self):
    ...
    def _read_from_control_interface(self):
        ...
        with open(f"{self.ANKAIOS_CONTROL_INTERFACE_BASE_PATH}/input", "rb") as f:
            ...
            while True:
                ...
                if from_ankaios.response.requestId == self.REQUEST_ID:
                    self.logger.info(f"Receiving Response containing the workload states of the current state:\nFromServer {{\n{from_ankaios}}}\n")
                else:
                    self.logger.info(f"RequestId does not match. Skipping messages from requestId: {request_id}")

    def write_to_control_interface(self):
        ...
        with open(f"{self.ANKAIOS_CONTROL_INTERFACE_BASE_PATH}/output", "ab") as f:
            update_workload_request = self.create_request_to_add_new_workload()
            ...
            self.logger.info(f'Sending Request containing details for adding the dynamic workload \"dynamic_nginx\":\nToServer {{\n{update_workload_request}}}\n')
            ...
            request_complete_state = self.create_request_for_complete_state()
            ...
            while True:
                self.logger.info(f"Sending Request containing details for requesting all workload states:\nToServer {{{request_complete_state}}}\n")
                ...
                f.flush()
                time.sleep(self.WAITING_TIME_IN_SEC) # Wait according to WAITING_TIME_IN_SEC until sending the next Request to Ankaios to avoid spamming...
