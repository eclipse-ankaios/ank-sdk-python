...
class CompleteState:
    ...
    def set_workload(self, name: str, workload: Workload) -> None:
    ...
    def get_workloads(self) -> dict[str, Workload]:
        """Get a workloads from the complete state."""
    ...
    def _to_proto(self) -> _ank_base.CompleteState:
        ...
        for name, workload in self._workloads.items():
            self._complete_state.desiredState.workloads.workloads[name].CopyFrom(workload._to_proto())
    ...
    def _from_proto(self, proto: _ank_base.CompleteState) -> None:
        ...
        self._workloads = {}
        for name, workload in self._complete_state.desiredState.workloads.workloads.items():
            self._workloads[name] = Workload()
            self._workloads[name]._from_proto(workload)
...
if __name__ == "__main__":
    ...
    # Create workload
    workload = Workload()
    workload.update_agent_name("agent_A")
